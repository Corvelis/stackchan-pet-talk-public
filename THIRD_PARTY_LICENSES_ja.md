...
TestFlight と Google Play の限定ベータビルドでは、以下を守ってください。

- アプリ内のライセンス画面を含める。
- `LICENSE`、`NOTICE`、同梱コンポーネントの通知をアプリのライセンスレジストリに含める。
  画面上では、同梱モデル、辞書、ランタイムの通知を `StackChan Pet Talk` のライセンス項目に集約する。
- ベータビルドをパッケージする前に `bash scripts/check_release_assets.sh` を実行する。
- manifest を更新して確認するまでは、同梱される `.onnx`、`.so`、`.dylib`、`.bin`、`.dic`、辞書 `.json` を追加・置換しない。
- 公開配布前には、Piper Plus 辞書の生成元 commit / package version と、LiteRT-LM バイナリの release / build provenance を固定する。
