...
## Beta Distribution Notes

For TestFlight and Google Play limited beta builds:

- Include the in-app Licenses screen.
- Include `LICENSE`, `NOTICE`, and bundled component notices in the app license
  registry. In the UI, consolidate bundled model, dictionary, and runtime
  notices under the `StackChan Pet Talk` license entry.
- Run `bash scripts/check_release_assets.sh` before packaging beta builds.
- Do not add or replace bundled `.onnx`, `.so`, `.dylib`, `.bin`, `.dic`, or
  dictionary `.json` files unless the manifest is updated and reviewed.
- Before public distribution, pin the exact Piper Plus dictionary generation
  commit/package version and LiteRT-LM binary release/build provenance.
