...
- `設定 > 音声AI > TTS エンジン > Style-Bert-VITS2`

必要なファイル:

- Core ML 向けに変換・分割済みの BERT フォルダ
- Core ML 向けに変換・分割済みの VITS2 音声フォルダ

アプリでは `BERT フォルダ` と `VITS2 フォルダ` から、それぞれの
フォルダを指定します。VITS2 側に複数の話者やスタイルが含まれる場合は、取り込み後に
話者、スタイル、話速を選べます。

通常の Style-Bert-VITS2 モデルをそのまま選ぶことはできません。配布者がこのアプリの
iPhone 実装に対応する Core ML 形式へ事前に変換し、必要な BERT/VITS2 ファイルを
フォルダ単位で用意する必要があります。アプリ内でのモデルダウンロードや変換には
対応していません。

モデルごとにライセンスと話者・キャラクターの利用条件が異なります。配布前と利用前に、
変換元モデルのモデルカード、音声利用条件、再配布条件を確認してください。
...
選択したモデルはアプリ管理領域へ取り込まれるため、取り込み完了後は元ファイルを
移動または削除しても、アプリ内のコピーが使われます。
...
- 選択した GGUF と mmproj はアプリ管理領域へ取り込まれます。

iPhone で対応する gpt-oss Flash MoE 形式を使う場合は、メイン GGUF を選んだあと、対応する
レイヤーパックのフォルダを選択します。フォルダには同じモデルから作成された
`.layer00.pack` から `.layer23.pack` までの 24 ファイルが必要です。未取り込み layer が
表示された場合は完全な pack を選び直し、別のモデルや revision の layer を混ぜないでください。

`取り込んだLLMファイルを削除` を実行すると、アプリ管理領域にコピーした
モデルと保存済みの選択情報を削除します。元のダウンロードファイルは削除されません。
